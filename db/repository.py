...
class Repository(Generic[T]):
    ...
    def query_groupby(self):
        agg_result= self.collection.aggregate(
            [{
            "$group" : 
                {"_id" : "$table.$id", 
        ...
        return sorted(data, key=lambda x: x["suma"]) 
    ...
    def get_all(self):
        results = self.collection.find().sort("votes", -1)
        data = []
        for r in results:
            r["_id"] = r["_id"].__str__()
            r = self.transform_object_ids(r)
    ...
    def get_by_id(self, id):
        return self.get_by_key("_id", id)

    def get_by_key(self, key, value):
        if key=="_id":
            result = self.collection.find_one({"_id": ObjectId(value)})
        else:
